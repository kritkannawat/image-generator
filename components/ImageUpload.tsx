  ...
  const onDrop = useCallback(
    (acceptedFiles: File[], fileRejections) => {
      ...
      reader.onerror = (error) => {
        onError?.("เกิดข้อผิดพลาดในการอ่านไฟล์ กรุณาลองอีกครั้ง");
        setIsLoading(false);
      };
    ...
    <div className="w-full">
      {!currentImage ? (
        <div
          ...
          ${isDragActive ? "bg-secondary/50" : "bg-foreground/5"}
          ${isLoading ? "opacity-50 cursor-wait" : ""}
          transition-colors duration-200 ease-in-out hover:bg-secondary
          ...
          <div className="flex flex-row items-center" role="presentation">
            ...
            <div className="">
              <p className="text-sm font-medium text-foreground">
                วางรูปรูปภาพของคุณที่นี่หรือคลิกเพื่อเรียกดู
              </p>
          ...
          <div className="flex w-full items-center mb-4">
            ...
            <div className="flex-grow min-w-0">
              <p className="text-sm font-medium truncate text-foreground">
                {selectedFile?.name || "รูปภาพปัจจุบัน"}
              </p>
              {selectedFile && (
                <p className="text-xs text-muted-foreground">
